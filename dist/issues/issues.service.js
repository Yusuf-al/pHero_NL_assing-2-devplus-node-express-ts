...
const getSingleIssue = async (payload) => {
    const { id } = payload;
    let query = `
    SELECT 
      *
...
const deleteSingleIssue = async (payload) => {
    const { id } = payload;
...
const updateIssue = async (payload) => {
  ...
  `;
    ...
    const issue = rows[0];
    const isMaintainerOwner = user.role === "maintainer" &&
        issue.status === "open" &&
        issue.reporter_id === user.id;
    const isContributor = user.role === "contributor";
    if (!isMaintainerOwner && !isContributor) {
        throw new Error("Unauthorized: You do not have permission to update this issue");
    }
    const updateQuery = `
    UPDATE issues
    SET
